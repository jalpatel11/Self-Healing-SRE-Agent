...
def investigator_agent(state: SREAgentState) -> dict:
    ...
    """
    print("\n🔍 INVESTIGATOR AGENT: Starting log analysis...")
...
Please reconsider the root cause analysis with this feedback in mind. The previous fix didn't work correctly."""
    ...
    if root_cause_found:
        print(f"   ✅ Root cause found: {analysis_text[:100]}...")
    else:
        print(f"   ⚠️  Root cause unclear, may need another iteration")
...
def mechanic_agent(state: SREAgentState) -> dict:
    ...
    """
    print("\n🔧 MECHANIC AGENT: Generating code fix...")
...
Respond with ONLY the Python code, no explanations before or after."""
    ...
    print(f"   ✅ Generated fix ({len(fix_code)} characters)")
...
def validator_node(state: SREAgentState) -> dict:
    ...
    """
    print("\n✅ VALIDATOR NODE: Testing the generated fix...")
    
    fix_code = state.get("fix_code", "")
    
    if not fix_code:
        print("   ❌ No fix code to validate!")
        return {
            "fix_validated": False,
...
def pr_creator_node(state: SREAgentState) -> dict:
    ...
    """
    print("\n📝 PR CREATOR NODE: Opening GitHub Pull Request...")
    
    fix_code = state.get("fix_code", "")
    root_cause = state.get("root_cause_analysis", "")
    
    # Prepare PR title and body
    title = "🤖 Self-Healing Agent: Fix KeyError in /api/data endpoint"
...
"""
    ...
    if "✅" in pr_result or "PR URL:" in pr_result or "Simulated PR" in pr_result:
